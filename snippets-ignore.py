...
def get_snippet_entries(project_id, version_id):
    param_string = "?filter=bomMatchType:snippet&filter=bomMatchReviewStatus:not_reviewed&filter=bomMatchInclusion:false"
...
def send_bulk_update_request(project_id, version_id, snippet_updates):
    ...
    # jsondump = json.dumps(snippet_updates)
    # print(jsondump)
    try:
        response = bd.session.put(url, json=snippet_updates, headers=headers)
        # time.sleep(1)
        # response = bd.session.put(url, json=jsondump, headers=headers)
    except requests.HTTPError as err:
        bd.http_error_handler(err)
        exit()

    # if response.status_code == 412:
    #     print(snippet_updates)
    #     # time.sleep(2)
    #     # response = bd.session.put(url, json=snippet_updates, headers=headers)
...
def create_snippet_update_body(snippet_bom_entry, match, ignore):
    # print(snippet_bom_entry)
    # print(snippet_bom_entry['fileSnippetBomComponents'][0]['ignored'])
    return {
        ...
        "sourceStartLines": match['sourceStartLines'],
        "sourceEndLines": match['sourceEndLines'],
        "hashId": match['hashId'],
...
for snippet_item in snippet_bom_entries:
    ...
    for match in snippet_item['fileSnippetBomComponents']:
        ...
        if not args.report and len(snippetUpdates) >= 100:
            print("Send request to ignore 100 snippets.")
